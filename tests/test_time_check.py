...
from time_check import is_in_restricted_hours, is_in_restricted_hours_for_today
import datetime

def test_is_in_restricted_hours_within_range():
    test_time = datetime.time(10, 30)
    with patch('time_check.datetime.datetime') as mock_datetime_class:
...
def test_is_in_restricted_hours_for_today_workday():
    with patch('time_check.get_date_type') as mock_get_date_type:
